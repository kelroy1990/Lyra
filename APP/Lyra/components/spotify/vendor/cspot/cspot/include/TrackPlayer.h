...
class TrackPlayer : bell::Task {
 ...
 private:
  ...
  // Set while runTask() is blocked in loadedSemaphore->twait().
  // stop() gives this semaphore to unblock the wait immediately.
  std::shared_ptr<QueuedTrack> waitingTrack;
  std::mutex waitingTrackMutex;
