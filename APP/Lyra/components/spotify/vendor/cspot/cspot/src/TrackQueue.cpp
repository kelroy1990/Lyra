...
void QueuedTrack::stepLoadCDNUrl(const std::string& accessKey) {
  ...
  try {
    ...
    // api.spotify.com/v1/storage-resolve is deprecated and returns HTTP 429.
    // Current endpoint is spclient.wg.spotify.com (same Bearer token works).
    std::string requestUrl = string_format(
        "https://spclient.wg.spotify.com/storage-resolve/v2/files/audio/interactive/10/"
    ...
    if (jsonResult == nullptr) {
      CSPOT_LOG(error, "CDN URL: JSON parse failed. Body: %.200s", result.data());
      throw std::runtime_error("CDN URL JSON parse failed");
    }
    cJSON* cdnUrlArray = cJSON_GetObjectItem(jsonResult, "cdnurl");
    cJSON* cdnUrlItem  = cJSON_GetArrayItem(cdnUrlArray, 0);
    if (cdnUrlItem == nullptr || cdnUrlItem->valuestring == nullptr) {
      CSPOT_LOG(error, "CDN URL: 'cdnurl' missing or empty. Body: %.300s", result.data());
      cJSON_Delete(jsonResult);
      throw std::runtime_error("CDN URL not found in response");
    }
    ...
    if (!jsonResult.contains("cdnurl") || jsonResult["cdnurl"].empty()) {
      CSPOT_LOG(error, "CDN URL: 'cdnurl' missing. Body: %.300s", result.data());
      throw std::runtime_error("CDN URL not found in response");
    }
