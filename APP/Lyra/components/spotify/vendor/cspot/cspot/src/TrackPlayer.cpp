...
void TrackPlayer::stop() {
  isRunning = false;
  resetState();
  // If runTask() is blocked in loadedSemaphore->twait(), unblock it now so
  // stop() doesn't freeze handlePacket() for up to 10 seconds.
  {
    std::scoped_lock wl(waitingTrackMutex);
    if (waitingTrack) {
      waitingTrack->loadedSemaphore->give();
      waitingTrack = nullptr;
    }
  }
  std::scoped_lock lock(runningMutex);
}
...
void TrackPlayer::runTask() {
  ...
  while (isRunning) {
    ...
    if (track->state != QueuedTrack::State::READY) {
      ...
      track->loadedSemaphore->twait(10000);
      {
        std::scoped_lock wl(waitingTrackMutex);
        waitingTrack = nullptr;
      }
